...
- **사용처**: 레이어드 C# 제어 코드 예시 (`csharp/AutomationTemplate/`)의 레이어 구조(0_System~4_Process)에 맞춘 Few-shot / 프롬프트 예시.
...
8. **사이클 종료 및 반복 여부 결정**
   ...
   - Stop 버튼이 눌리면 **가동 중인 단계에서 일시 정지**되고, Start 버튼을 누르면 **기존 step부터 재개**한다. (예: `4_Process/PHandler.cs`의 step 상태 유지)

### 4.2 오류/인터록 개념

- PartPresentSensor가 OFF인 상태에서 Start 버튼을 누르면 알람을 발생시키고 사이클을 시작하지 않는다.
- 이동 중 Stop 버튼을 누르면 **알람이 아니라 일시 정지**로 처리할 수 있으며, Start 버튼으로 재개 가능하다. (위 정책은 설비/안전요구에 따라 조정)
